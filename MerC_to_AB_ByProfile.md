...
| `layout-c-payload.example.json` | ตัวอย่าง JSON ฝั่ง C สำหรับเทียบ path |

**อ่านยังไง:** ของร่วมก่อน → router กลุ่ม A/B/C → เปิดไฟล์ profile ที่รับผิดชอบ  
ในแต่ละไฟล์มีหัวข้อ **「อ่านก่อนแปลง (ภาษาคน)」** อ่านก่อน แล้วค่อยลงตารางรายฟิลด์ (คอลัมน์ **อ่านยังไง** = C เป็นแบบนี้ → AB ได้อะไร พร้อมตัวอย่าง)  
ตารางเป็น **JSON C path → JSON AB path** ไม่ใช่ชื่อคอลัมน์ Excel
...
`MATERIALS[*].salesPricePromotion` → `get.fieldP` ·  
`MATERIALS[*].discountAmount` → `get.fieldR` ·  
`MATERIALS[*].discountPercentPlu` → `get.field22`  
(อันแรกที่มี)
