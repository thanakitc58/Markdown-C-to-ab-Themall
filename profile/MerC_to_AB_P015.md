...
| `bonusBuyProfile` ≠ `P015` | ไม่เข้า router นี้ |
...
| `HEADER.bonusBuyProfile` | `bonusBuyProfile` | Bonus Buy Profile | **calc** | `shortCode` → `"P015"` |
...
| จาก C (JSON) | → AB (JSON) | ชื่อใน To-Be | req | การทำ | เงื่อนไข P015 |
|--------------|-------------|--------------|-----|-------|----------------|
| `MATERIALS[*].numberOfBonusBuy` / `noof_bonus_buy` | `bonusBuyNumber` | Bonus buy No. | M | **copy** | default `"1"` |
| `MATERIALS[*].bonusBuyProfile` / `bonus_buy_profile` | `bonusBuyProfile` | Bonus Buy Profile | — | **calc** | `"P015"` |
...
| `HEADER.onlineDescriptionEnglish` / `MATERIALS[*].onlineDescriptionEnglish` | `onlineDescriptionEnglish` | Online Description EN | — | **omit** | P015 **ไม่ใส่** |
| `HEADER.onlineDescriptionThai` / `MATERIALS[*].onlineDescriptionThai` | `onlineDescriptionThai` | Online Description TH | — | **omit** | P015 **ไม่ใส่** |
| `MATERIALS[*].referenceCode` | `referenceCode` | Reference code | — | **omit** | P015 **ไม่ใส่** |
...
P015 เป็น **Get-only** → `buy = []` เสมอ  
field ใน buy ทั้งหมด = **ไม่ใช้** สำหรับ profile นี้

| จาก C (JSON) | → AB `buy[0]` | ชื่อใน To-Be | req | การทำ | เงื่อนไข P015 |
|--------------|---------------|--------------|-----|-------|----------------|
...
| จาก C (JSON) | → AB `get[0]` | ชื่อใน To-Be | req | การทำ | เงื่อนไข P015 |
|--------------|---------------|--------------|-----|-------|----------------|
...
| `discountPercentForP015` / `discountPercentPlu` / `discountPct` | `field22` | ส่วนลด % [%] | — | **calc** | ลำดับ 3 + `normalizePctFull` |
...
| 3 | `discountPercentForP015` / `discountPercentPlu` / `discountPct` | `field22` | เมื่อไม่มี 1 และ 2 · ใช้ `normalizePctFull` |
