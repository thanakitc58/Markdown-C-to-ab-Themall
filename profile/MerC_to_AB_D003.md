...
| `bonusBuyProfile` ≠ `D003` | ไม่เข้า router นี้ |
...
| `HEADER.bonusBuyProfile` | `bonusBuyProfile` | Bonus Buy Profile | **calc** | `shortCode` → `"D003"` |
...
| จาก C (JSON) | → AB (JSON) | ชื่อใน To-Be | req | การทำ | เงื่อนไข D003 |
|--------------|-------------|--------------|-----|-------|----------------|
| `MATERIALS[*].numberOfBonusBuy` / `noof_bonus_buy` | `bonusBuyNumber` | Bonus buy No. | M | **copy** | default `"1"` |
| `MATERIALS[*].bonusBuyProfile` / `bonus_buy_profile` | `bonusBuyProfile` | Bonus Buy Profile | — | **calc** | `"D003"` |
...
| `MATERIALS[*].referenceCode` | `referenceCode` | Reference code | — | **omit** | D003 **ไม่ใส่** |
...
> **หมายเหตุ D003:** ตาราง §6 ใช้เมื่อ element เป็นฝั่ง buy · ตาราง §7 ใช้เมื่อ element เป็นฝั่ง get · อีกฝั่งเป็น `[]`


| จาก C (JSON) | → AB `buy[0]` | ชื่อใน To-Be | req | การทำ | เงื่อนไข D003 |
|--------------|---------------|--------------|-----|-------|----------------|
...
| จาก C (JSON) | → AB `get[0]` | ชื่อใน To-Be | req | การทำ | เงื่อนไข D003 |
|--------------|---------------|--------------|-----|-------|----------------|
...
| `salesPriceNormal` / `sales_price_normal` | `field8` | **ราคาขายปกติ** | — | **omit** | **D003 ห้ามใส่** (กลุ่ม B/C) |
| — | `vat` | VAT | — | **omit** | |
...
### 10.7 แยกฝั่งตาม `mechanic` (สำคัญ)

```
ถ้า mechanic ชี้ buy / suffix (A) / A(Coupon)+B(A)
  → element: { buy: [mapBuy], get: [] }

ถ้า mechanic ชี้ get / suffix (B) / +B(B)
  → element: { buy: [], get: [mapGet] }   // ไม่ใส่ get.field8
```
