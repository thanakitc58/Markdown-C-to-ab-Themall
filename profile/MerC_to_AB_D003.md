...
**อ่านยังไง:** ไฟล์นี้เป็น **สเปกครบทุก key ใน AB payload** สำหรับ D003 — แต่ละแถวบอกว่า map จาก C อย่างไร / ห้ามใส่ / เว้นว่าง / Auto  
...
convertLayoutCToAbPayload(c)
  ...
  BONUSBUYS = c.MATERIALS
    ...
    .filter(m => shortCode(m.bonusBuyProfile) === "D003")
    .flatMap(m => splitByMechanicD003(ctx))  // 1 แถวอาจได้ >1 element
...
## อ่านก่อนแปลง (ภาษาคน — D003)

**D003 อยู่กลุ่ม C (แยก element ตาม mechanic)**  
...
**Reference code** — D003 **ไม่ส่ง** (ห้ามมี key นี้ใน output)
...
| `bonusBuyProfile` ไม่ใช่ `D003` | ไม่เข้าไฟล์นี้ | แถวเป็น P001 แต่คุณเปิดสเปก D003 → ส่งไป router อื่น ไม่ใช่บั๊ก |
...
| จาก C (JSON) | → AB (JSON) | การทำ | เงื่อนไข D003 |
|--------------|-------------|-------|----------------|
| — | `LAYOUT` | **calc** | คงที่ `"AB"` |
| `STATUS` | `STATUS` | **copy** | ส่งตรง |
| `HEADER` | `HEADER` | **map** | §3 |
| `MATERIALS[*]` | `BONUSBUYS[*]` | **calc** | สร้างใหม่ §4–§7 · splitByMechanic — buy-only หรือ get-only คนละ element |
...
HEADER คือหัวโปรทั้งใบ — **ใช้ util ร่วม** (`mapHeaderShared`) ไม่ต้องแยก if D003  
...
| `bonusBuyProfile` | `bonusBuyProfile` | **calc** | อยากได้รหัสสั้น · ตัวอย่าง: `"D001 - Same group"` → `"D003"` |
...
| `bonusBuyHeader` | §5 |
| `buy[]` | `[1]` หรือ `[]` ตาม mechanic — §6 |
| `get[]` | `[1]` หรือ `[]` ตาม mechanic — §7 |
| `stores[]` | §7.1 |
| `card[]` | **empty** |
| `tender[]` | **empty** |
| `installment[]` | **empty** |
...
| `bonusBuyProfile` | `bonusBuyProfile` | Bonus Buy Profile | — | **calc** | ตัดให้เหลือรหัสสั้น `"D003"` เท่านั้น |
...
| `referenceCode` | `referenceCode` | Reference | — | **omit** | **D003 ไม่ส่ง** · แม้ C มีค่า ก็ตัดทิ้ง |
...
| `salesPriceNormal` | `field8` | ราคาขายปกติ | **omit** | **ห้ามมี `field8` ใน get** · ราคาปกติไปอยู่ `buy[0].field8` แล้ว · ถ้าใส่ทั้งสองฝั่งจะผิดสเปก D003 |
...
### 10.4 ปรับสเกล % แบบเต็ม (`normalizePctFull`) — ใช้ใน D003
...
| 3 | ส่วนลด % → `get.field22` ตาม normalize ของ D003 |
...
## 13) Checklist (D003 ครบ)

สถานะเทียบ `convertLayoutCToAbc` ใน `constants/promotion-payload.js` (entry `if (profile === "D003")`)

### โครง
- [x] `LAYOUT` = `"AB"`
- [x] `MATERIALS` ฝั่ง AB = `[]`
- [x] parse `mechanic` ก่อนตัดสิน buy หรือ get
- [x] ไม่บังคับ buy+get ใน element เดียว
- [x] % normalize เต็ม
- [x] มี `validTime*`
...
| หัวข้อ | หมายเหตุ |
|--------|----------|
| D003 Coupon merge ก้อนก่อนหน้า | ตอนนี้ 1 แถว = 1 ก้อน · ยังไม่รวม `(B)` เข้า `(A)` |
...
เทียบ: `payload_AB_Reference.md` · `Gen_D003` / `D003_Header` ใน `Mer-C_Convert_To_STD.txt` · `layout-ab-payload.example.json`
...
| แยก buy/get ตาม mechanic | ✅ ตาม `Gen_D003` |
