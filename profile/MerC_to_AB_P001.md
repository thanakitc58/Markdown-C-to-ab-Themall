...
| `bonusBuyProfile` ≠ `P001` | ไม่เข้า router นี้ |
...
| `HEADER.bonusBuyProfile` | `bonusBuyProfile` | Bonus Buy Profile | **calc** | `shortCode` → `"P001"` |
...
| จาก C (JSON) | → AB (JSON) | ชื่อใน To-Be | req | การทำ | เงื่อนไข P001 |
|--------------|-------------|--------------|-----|-------|----------------|
| `MATERIALS[*].numberOfBonusBuy` / `noof_bonus_buy` | `bonusBuyNumber` | Bonus buy No. | M | **copy** | default `"1"` |
| `MATERIALS[*].bonusBuyProfile` / `bonus_buy_profile` | `bonusBuyProfile` | Bonus Buy Profile | — | **calc** | `"P001"` |
...
| `HEADER.onlineDescriptionEnglish` / `MATERIALS[*].onlineDescriptionEnglish` | `onlineDescriptionEnglish` | Online Description EN | — | **omit** | P001 **ไม่ใส่** |
| `HEADER.onlineDescriptionThai` / `MATERIALS[*].onlineDescriptionThai` | `onlineDescriptionThai` | Online Description TH | — | **omit** | P001 **ไม่ใส่** |
| `MATERIALS[*].referenceCode` | `referenceCode` | Reference code | — | **omit** | P001 **ไม่ใส่** |
...
P001 เป็น **Get-only** → `buy = []` เสมอ  
field ใน buy ทั้งหมด = **ไม่ใช้** สำหรับ profile นี้

| จาก C (JSON) | → AB `buy[0]` | ชื่อใน To-Be | req | การทำ | เงื่อนไข P001 |
|--------------|---------------|--------------|-----|-------|----------------|
...
| จาก C (JSON) | → AB `get[0]` | ชื่อใน To-Be | req | การทำ | เงื่อนไข P001 |
|--------------|---------------|--------------|-----|-------|----------------|
