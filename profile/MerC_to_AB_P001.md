...
**อ่านยังไง:** ไฟล์นี้เป็น **สเปกครบทุก key ใน AB payload** สำหรับ P001 — แต่ละแถวบอกว่า map จาก C อย่างไร / ห้ามใส่ / เว้นว่าง / Auto  
...
## อ่านก่อนแปลง (ภาษาคน — P001)

**P001 อยู่กลุ่ม A (Get-only)**  
...
**Reference code** — P001 **ไม่ส่ง** (ห้ามมี key นี้ใน output)

**Online EN/TH** — P001 ไม่ใส่
...
| `bonusBuyProfile` ไม่ใช่ `P001` | ไม่เข้าไฟล์นี้ | แถวเป็น P001 แต่คุณเปิดสเปก P001 → ส่งไป router อื่น ไม่ใช่บั๊ก |
...
HEADER คือหัวโปรทั้งใบ — **ใช้ util ร่วม** (`mapHeaderShared`) ไม่ต้องแยก if P001  
...
| `bonusBuyProfile` | `bonusBuyProfile` | **calc** | อยากได้รหัสสั้น · ตัวอย่าง: `"D001 - Same group"` → `"P001"` |
...
| `bonusBuyProfile` | `bonusBuyProfile` | Bonus Buy Profile | — | **calc** | ตัดให้เหลือรหัสสั้น `"P001"` เท่านั้น |
...
| online EN | `onlineDescriptionEnglish` | Online EN | — | **omit** | **P001 ไม่ใช้ online description** → ไม่ส่ง key นี้ |
| online TH | `onlineDescriptionThai` | Online TH | — | **omit** | **P001 ไม่ใช้ online description** → ไม่ส่ง key นี้ |
| `referenceCode` | `referenceCode` | Reference | — | **omit** | **P001 ไม่ส่ง** · แม้ C มีค่า ก็ตัดทิ้ง |
...
### 10.4 ปรับสเกล % แบบเต็ม (`normalizePctFull`) — ใช้ใน P001
...
## 13) Checklist (P001 ครบ)

สถานะเทียบ `convertLayoutCToAbc` ใน `constants/promotion-payload.js` (entry `if (profile === "P001")`)
...
เทียบ: `payload_AB_Reference.md` · `Gen_P001` / `P001_Header` ใน `Mer-C_Convert_To_STD.txt` · `layout-ab-payload.example.json`
