...
| `bonusBuyProfile` ≠ `D002` | ไม่เข้า router นี้ |
...
| `HEADER.bonusBuyProfile` | `bonusBuyProfile` | Bonus Buy Profile | **calc** | `shortCode` → `"D002"` |
...
| จาก C (JSON) | → AB (JSON) | ชื่อใน To-Be | req | การทำ | เงื่อนไข D002 |
|--------------|-------------|--------------|-----|-------|----------------|
| `MATERIALS[*].numberOfBonusBuy` / `noof_bonus_buy` | `bonusBuyNumber` | Bonus buy No. | M | **copy** | default `"1"` |
| `MATERIALS[*].bonusBuyProfile` / `bonus_buy_profile` | `bonusBuyProfile` | Bonus Buy Profile | — | **calc** | `"D002"` |
...
| `MATERIALS[*].referenceCode` | `referenceCode` | Reference code | — | **omit** | D002 **ไม่ใส่** |
...
| จาก C (JSON) | → AB `buy[0]` | ชื่อใน To-Be | req | การทำ | เงื่อนไข D002 |
|--------------|---------------|--------------|-----|-------|----------------|
...
| จาก C (JSON) | → AB `get[0]` | ชื่อใน To-Be | req | การทำ | เงื่อนไข D002 |
|--------------|---------------|--------------|-----|-------|----------------|
...
| `salesPriceNormal` / `sales_price_normal` | `field8` | **ราคาขายปกติ** | — | **omit** | **D002 ห้ามใส่** (กลุ่ม B/C) |
| — | `vat` | VAT | — | **omit** | |
