...
| `bonusBuyProfile` ≠ `P010` | ไม่เข้า router นี้ |
...
| `HEADER.bonusBuyProfile` | `bonusBuyProfile` | Bonus Buy Profile | **calc** | `shortCode` → `"P010"` |
...
| จาก C (JSON) | → AB (JSON) | ชื่อใน To-Be | req | การทำ | เงื่อนไข P010 |
|--------------|-------------|--------------|-----|-------|----------------|
| `MATERIALS[*].numberOfBonusBuy` / `noof_bonus_buy` | `bonusBuyNumber` | Bonus buy No. | M | **copy** | default `"1"` |
| `MATERIALS[*].bonusBuyProfile` / `bonus_buy_profile` | `bonusBuyProfile` | Bonus Buy Profile | — | **calc** | `"P010"` |
| `MATERIALS[*].mechanic` | `mechanic` | Mechanic | — | **copy** | + ใช้ lookup qty §10 |
| `HEADER.timeFrom` | `validTimeFrom` | Valid Time from | — | **omit** | P010 **ไม่ใส่** `validTime*` |
| `HEADER.timeTo` | `validTimeTo` | Valid Time to | — | **omit** | P010 **ไม่ใส่** `validTime*` |
...
| `HEADER.referenceCode` / `MATERIALS[*].referenceCode` | `referenceCode` | Reference code | — | **copy** | **ใส่** |
...
P010 เป็น **Get-only** → `buy = []` เสมอ  
field ใน buy ทั้งหมด = **ไม่ใช้** สำหรับ profile นี้

| จาก C (JSON) | → AB `buy[0]` | ชื่อใน To-Be | req | การทำ | เงื่อนไข P010 |
|--------------|---------------|--------------|-----|-------|----------------|
...
| จาก C (JSON) | → AB `get[0]` | ชื่อใน To-Be | req | การทำ | เงื่อนไข P010 |
|--------------|---------------|--------------|-----|-------|----------------|
