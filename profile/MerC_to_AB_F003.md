...
| `bonusBuyProfile` ≠ `F003` | ไม่เข้า router นี้ |
...
| `HEADER.bonusBuyProfile` | `bonusBuyProfile` | Bonus Buy Profile | **calc** | `shortCode` → `"F003"` |
...
| จาก C (JSON) | → AB (JSON) | ชื่อใน To-Be | req | การทำ | เงื่อนไข F003 |
|--------------|-------------|--------------|-----|-------|----------------|
| `MATERIALS[*].numberOfBonusBuy` / `noof_bonus_buy` | `bonusBuyNumber` | Bonus buy No. | M | **copy** | default `"1"` |
| `MATERIALS[*].bonusBuyProfile` / `bonus_buy_profile` | `bonusBuyProfile` | Bonus Buy Profile | — | **calc** | `"F003"` |
...
| `MATERIALS[*].referenceCode` | `referenceCode` | Reference code | — | **omit** | F003 **ไม่ใส่** |
...
> **หมายเหตุ F003:** ตาราง §6 ใช้เมื่อ mechanic ลงท้าย `(A)` · ตาราง §7 ใช้เมื่อลงท้าย `(B)` · อีกฝั่งเป็น `[]`


| จาก C (JSON) | → AB `buy[0]` | ชื่อใน To-Be | req | การทำ | เงื่อนไข F003 |
|--------------|---------------|--------------|-----|-------|----------------|
...
| จาก C (JSON) | → AB `get[0]` | ชื่อใน To-Be | req | การทำ | เงื่อนไข F003 |
|--------------|---------------|--------------|-----|-------|----------------|
...
| `salesPriceNormal` / `sales_price_normal` | `field8` | **ราคาขายปกติ** | — | **omit** | **F003 ห้ามใส่** (กลุ่ม B/C) |
| — | `vat` | VAT | — | **omit** | |
...
คนละ index/element — อย่าบังคับ buy+get ใน element เดียว
