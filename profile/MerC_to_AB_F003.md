...
**อ่านยังไง:** ไฟล์นี้เป็น **สเปกครบทุก key ใน AB payload** สำหรับ F003 — แต่ละแถวบอกว่า map จาก C อย่างไร / ห้ามใส่ / เว้นว่าง / Auto  
...
convertLayoutCToAbPayload(c)
  ...
  BONUSBUYS = c.MATERIALS
    ...
    .filter(m => shortCode(m.bonusBuyProfile) === "F003")
    .flatMap(m => splitByMechanicF003(ctx))
...
## อ่านก่อนแปลง (ภาษาคน — F003)

**F003 อยู่กลุ่ม C (แยก element ตาม mechanic)**  
User กรอกแถวใน `MATERIALS` เหมือนกลุ่ม B แต่ตอนแปลงจะ **แยกเป็นคนละก้อน BONUSBUY**:
- mechanic ลงท้าย `(A)` → ก้อนที่มีแค่ **`buy`** (`get` ว่าง)
- mechanic ลงท้าย `(B)` → ก้อนที่มีแค่ **`get`** (`buy` ว่าง)
ไม่เอามายัด buy+get ในก้อนเดียวแบบ D001
...
**ส่วนลด %** ใช้แบบง่าย: เอาค่าที่กรอก **หาร 100 เสมอ** (เช่น `16.68` → `0.1668`)

**เวลา Valid** ใส่ใน `bonusBuyHeader` · ถ้าว่างให้เป็นทั้งวัน `00:00:00`–`23:59:59`

**Reference code** — F003 **ไม่ส่ง** (ห้ามมี key นี้ใน output)
...
| `bonusBuyProfile` ไม่ใช่ `F003` | ไม่เข้าไฟล์นี้ | แถวเป็น P001 แต่คุณเปิดสเปก F003 → ส่งไป router อื่น ไม่ใช่บั๊ก |
...
| จาก C (JSON) | → AB (JSON) | การทำ | เงื่อนไข F003 |
|--------------|-------------|-------|----------------|
| — | `LAYOUT` | **calc** | คงที่ `"AB"` |
| `STATUS` | `STATUS` | **copy** | ส่งตรง |
| `HEADER` | `HEADER` | **map** | §3 |
| `MATERIALS[*]` | `BONUSBUYS[*]` | **calc** | สร้างใหม่ §4–§7 · splitByMechanic — (A) buy-only / (B) get-only |
...
HEADER คือหัวโปรทั้งใบ — **ใช้ util ร่วม** (`mapHeaderShared`) ไม่ต้องแยก if F003  
...
| `bonusBuyProfile` | `bonusBuyProfile` | **calc** | อยากได้รหัสสั้น · ตัวอย่าง: `"D001 - Same group"` → `"F003"` |
...
| `bonusBuyHeader` | §5 |
| `buy[]` | `[1]` หรือ `[]` ตาม suffix — §6 |
| `get[]` | `[1]` หรือ `[]` ตาม suffix — §7 |
| `stores[]` | §7.1 |
| `card[]` | **empty** |
| `tender[]` | **empty** |
| `installment[]` | **empty** |
...
| `bonusBuyProfile` | `bonusBuyProfile` | Bonus Buy Profile | — | **calc** | ตัดให้เหลือรหัสสั้น `"F003"` เท่านั้น |
| `mechanic` | `mechanic` | Mechanic | — | **copy** | ส่งข้อความ mechanic ตามที่เลือกในฟอร์ม · ข้อความนี้เอาไปเปิดตาราง Mechanic เพื่อรู้ว่าซื้อกี่ชิ้น / ได้กี่ชิ้น · และดูท้ายข้อความ `(A)`/`(B)` เพื่อแยกก้อน buy หรือ get |
...
| `referenceCode` | `referenceCode` | Reference | — | **omit** | **F003 ไม่ส่ง** · แม้ C มีค่า ก็ตัดทิ้ง |
...
> **กลุ่ม C:** สร้าง `buy[0]` เฉพาะก้อนที่ mechanic ลงท้าย `(A)` · ถ้าเป็น `(B)` ให้ `buy: []`
...
> **กลุ่ม C:** สร้าง `get[0]` เฉพาะก้อน mechanic ลงท้าย `(B)` · ถ้าเป็น `(A)` ให้ `get: []`
...
| `salesPriceNormal` | `field8` | ราคาขายปกติ | **omit** | **ห้ามมี `field8` ใน get** · ราคาปกติไปอยู่ `buy[0].field8` แล้ว · ถ้าใส่ทั้งสองฝั่งจะผิดสเปก F003 |
...
| ช่อง % (`discountPercentPlu` / `discountPercentForP015` / `discountPct`) | `field22` | ส่วนลด % | **calc** | ลำดับ 3 · ใช้เมื่อไม่มี P และ R · ตัวอย่างแบบง่าย: กรอก `16.68` → ส่ง `0.1668` (หาร 100) |
...
## 10) สูตรและ helper (F003)

### 10.1 กฎเฉพาะ profile (สรุป)

| หัวข้อ | ค่า F003 |
|--------|----------|
| เขียนฝั่ง | `(A)` → buy-only · `(B)` → get-only คนละ element |
| `get.field8` | **omit** |
...
### 10.4 ปรับสเกล % แบบง่าย (`normalizePctSimple`) — ใช้ใน F003

หาร 100 ทุกครั้ง ไม่มีเงื่อนไขย่อย
...
| 3 | ส่วนลด % → `get.field22` ตาม normalize ของ F003 |
...
## 13) Checklist (F003 ครบ)

สถานะเทียบ `convertLayoutCToAbc` ใน `constants/promotion-payload.js` (entry `if (profile === "F003")`)

### โครง
- [x] `LAYOUT` = `"AB"`
- [x] `MATERIALS` ฝั่ง AB = `[]`
- [x] แยก index ตาม suffix `(A)` / `(B)`
- [x] % ใช้ `pct / 100` เหมือน F001 (`normalizePctSimple`)
- [x] มี `validTime*`
...
เทียบ: `payload_AB_Reference.md` · `Gen_F003` / `F003_Header` ใน `Mer-C_Convert_To_STD.txt` · `layout-ab-payload.example.json`
