...
## อ่านก่อนแปลง (ภาษาคน — F001)

**F001 อยู่กลุ่ม B (Buy + Get ในก้อนเดียว)**  
...
**Reference code** — F001 **ไม่ส่ง** (ห้ามมี key นี้ใน output)
...
| `bonusBuyProfile` ไม่ใช่ `F001` | ไม่เข้าไฟล์นี้ | แถวเป็น P001 แต่คุณเปิดสเปก F001 → ส่งไป router อื่น ไม่ใช่บั๊ก |
...
HEADER คือหัวโปรทั้งใบ — **ใช้ util ร่วม** (`mapHeaderShared`) ไม่ต้องแยก if F001  
...
| `bonusBuyProfile` | `bonusBuyProfile` | **calc** | อยากได้รหัสสั้น · ตัวอย่าง: `"D001 - Same group"` → `"F001"` |
...
| `bonusBuyProfile` | `bonusBuyProfile` | Bonus Buy Profile | — | **calc** | ตัดให้เหลือรหัสสั้น `"F001"` เท่านั้น |
...
| `referenceCode` | `referenceCode` | Reference | — | **omit** | **F001 ไม่ส่ง** · แม้ C มีค่า ก็ตัดทิ้ง |
...
| `salesPriceNormal` | `field8` | ราคาขายปกติ | **omit** | **ห้ามมี `field8` ใน get** · ราคาปกติไปอยู่ `buy[0].field8` แล้ว · ถ้าใส่ทั้งสองฝั่งจะผิดสเปก F001 |
...
### 10.4 ปรับสเกล % แบบง่าย (`normalizePctSimple`) — ใช้ใน F001
